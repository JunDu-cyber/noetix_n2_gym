...
class N2ParkourCfg(N2PerceptiveCfg):
    ...
    class terrain(N2PerceptiveCfg.terrain):
        ...
        # 上楼时通道高于外面的 0 高度低地，绕行要先下再爬回来本身就亏；下楼时通道沉到
        # 0 以下，那块低地反而成了比通道更高的平台，爬出去沿边沿走到终点照样拿 goal
        # 奖励，绕行由亏变赚——所以必须单独压深。
        # 不用"一整块平底深坑"：18 级降到 -3.3m 时，那种坑在刚出发处就是 3.7m 自由落体，
        # 早期策略会大量摔死，接触力惩罚和终止统计全被这一下打乱。0.3m 的平行沟靠奖励
        # 压制绕行：通道宽 1.4~1.6m，走到外面即 |dy|>0.7m > goal_pass_lateral_tol(0.5m)，
        # goal 永远不计数、指针原地卡住。
        ...
        # 站哪儿都行，那练的其实只是跨缝；缩到 0.45x0.50m 单脚周围只剩 ±0.125/±0.200m
        # 余量，才真的要挑落脚点。再小就撞地形分辨率：horizontal_scale=0.1m，0.25m 的
        # 梅花桩只有 2x2 格，高度图表达不出桩子的边，策略也扫不到。
        parkour_stone_len_range = [0.9, 0.45]
        parkour_stone_width_range = [1.0, 0.5]

        # 【缝宽与横向偏置按 N2 实测步态标定，不是拍脑袋】
        # model_1200 在平地路点列上 6416 次触地(前进 0.65m/s)：
        #   摆动距离(同脚两次触地的 x 位移) 0.282m   步长(触地时两脚 x 差) 0.129m
        #   步宽(两脚 y 间距) 0.199m
        # 缝宽上限 0.20m：摆动脚要跨过 缝 + 脚长(0.20m) = 0.40m，是自然摆动 0.282m 的
        # 1.4 倍——真能跌但跌得过去。原来的 0.40m 需要 0.57m 摆动 = 2.0 倍，跨不过去，
        # 这一列会从头到尾卡在难度 0~1。
        # 横向偏置上限 0.18m：最难档 1.8 倍步宽。原来的 0.40m 让连续两块石头横向错开
        # 0.8m，是步宽的 4 倍，等于要求劈叉。
        ...
        #
        # 【必须跳过 x=0】与基类注释里的 "without center line" 同一个道理。Isaac 侧
        # 是按索引直接查高度图，采样点落在机器人正下方无所谓；sim2sim 侧是向下发射线，
        # x=0 那一整行(8 个点)常年落在骨盆正下方，射线先打到机器人自己身上。实测默认
        # 站姿下该行有 3 个点读成 0.092m(脚底)和 0.586m(髋部)而不是 0.000，策略以为
        # 身前有半米高的障碍，动作饱和到 |a|~5，两秒内摔倒。
        # 躯干遮挡是【持续】的、脚的遮挡才是瞬时的，所以避开中心线就够：本栅格实测在
        # 站姿/左腿前摆/右腿前摆三种姿态下均为 0 点被遮挡(半格错开的方案摆动相仍有 2 点)。
        # 【改这一项 = 换观测空间】actor 与 critic 读的是同一份 measured_heights，
        # 维度不变所以不报错，但旧 checkpoint 全部失效、必须重训，且 sim2sim 的 yaml
        # 必须同步(见 sim2sim/configs/n2_parkour.yaml)。
        measured_points_x = [-0.30, -0.15, 0.15, 0.30, 0.45, 0.60,
