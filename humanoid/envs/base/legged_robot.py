...
class LeggedRobot(BaseTask):
    ...
    def check_termination(self):
        ...
        if self.cfg.env.enable_early_termination:
            self.fallen_buf = (self.root_states[:, 2] - self.terrain_h) < self.cfg.env.termination_height
    ...
    def _refresh_cmd_resample_time(self, env_ids):
        self.resampling_time[env_ids] = torch.randint(self.cfg.commands.resampling_time[0], self.cfg.commands.resampling_time[1], (len(env_ids), ), device=self.device, requires_grad=False)
    ...
    def _post_physics_step_callback(self):
        ...
        self.standing_cmd = torch.norm(self.commands[:, :3], dim=1) <= self.min_cmd_vel
    ...
    def _compute_torques(self, actions):
        ...
        return torch.clip(torques * self.motor_strength, -self.torque_limits, self.torque_limits)
    ...
    def _disturbance_robots(self):
        """ Random add disturbance force and torque to the robots.
        """
        random_probabilities = torch.rand(self.num_envs, device=self.device)                 # 更新 disturbance_state 和 remaining_steps
        new_disturbance_mask = (random_probabilities < self.cfg.domain_rand.disturbance_probabilities) & (~self.disturbance_state)     # 有 disturbance_probabilities 的概率开始施加干扰力，且未处于干扰阶段
        self.remaining_steps[new_disturbance_mask] = torch.randint( # 设定新的干扰持续步数
                                                                self.cfg.domain_rand.disturbance_interval[0], 
                                                                self.cfg.domain_rand.disturbance_interval[1], 
                                                                (new_disturbance_mask.sum(),), dtype=torch.int32, device=self.device
                                                            )
        self.remaining_steps[self.disturbance_state] -= 1           # 对于正在进行干扰的环境，remaining_steps 减少
        self.disturbance_state[self.remaining_steps <= 0] = False   # 当 remaining_steps 为 0 时，停止干扰
        self.disturbance_state[new_disturbance_mask] = True         # 将新的干扰状态设置为 True
        ...
        if new_disturbance_mask.any():
            ...
            new_disturbance_torque = torch.where(
                torch.rand((num_new_disturbances, 1), device=self.device) < 0.5,
                torch_rand_float(-self.cfg.domain_rand.push_torque_range[0], -self.cfg.domain_rand.push_torque_range[1], (num_new_disturbances, 3), device=self.device),
                torch_rand_float(self.cfg.domain_rand.push_torque_range[0], self.cfg.domain_rand.push_torque_range[1], (num_new_disturbances, 3), device=self.device)
            )

            # 使用 new_disturbance_mask 将新生成的 force 和 torque 分配到正确的机器人上
            self.disturbance_force[new_disturbance_mask, 0, :] = new_disturbance_force
            self.disturbance_torque[new_disturbance_mask, 0, :] = new_disturbance_torque

        # 对所有不处于干扰状态的机器人，force 和 torque 设为 0
        no_disturbance_mask = ~self.disturbance_state
        self.disturbance_force[no_disturbance_mask] = 0
        self.disturbance_torque[no_disturbance_mask] = 0

        # 应用 force 和 torque
        self.gym.apply_rigid_body_force_tensors(
            self.sim,
            forceTensor=gymtorch.unwrap_tensor(self.disturbance_force),
            torqueTensor=gymtorch.unwrap_tensor(self.disturbance_torque),
    ...
    def update_keyboard_events(self):
        self.keyboard_controller.update(self)
    ...
    def _init_buffers(self):
        ...
        self.commands_des = torch.zeros_like(self.commands)
        self.commands_scale = torch.tensor([self.obs_scales.lin_vel, self.obs_scales.lin_vel, self.obs_scales.ang_vel], device=self.device, requires_grad=False,) # TODO change this
        self.resampling_time = torch.randint(self.cfg.commands.resampling_time[0], self.cfg.commands.resampling_time[1], (self.num_envs, ), device=self.device, requires_grad=False)
        self.min_cmd_vel = self.cfg.commands.min_cmd_vel
        self.standing_cmd = torch.norm(self.commands[:, :3], dim=1) <= self.min_cmd_vel
        ...
        self.base_ang_vel = quat_rotate_inverse(self.base_quat, self.root_states[:, 10:13])
        self.projected_gravity = quat_rotate_inverse(self.base_quat, self.gravity_vec)
        self.reset_toggle = torch.ones(self.num_envs, dtype=torch.bool, device=self.device, requires_grad=False)
        # check frame stack
        if self.cfg.env.frame_stack is not None:
        ...
        ########################################## Randomize ##########################################
        self.Kp_factors = torch.ones_like(self.p_gains) 
        self.Kd_factors = torch.ones_like(self.d_gains)
        self.motor_strength = torch.ones_like(self.actions)

        self.rand_push_force = torch.zeros((self.num_envs, 3), dtype=torch.float32, device=self.device)
        self.rand_push_torque = torch.zeros((self.num_envs, 3), dtype=torch.float32, device=self.device)

        # disturbance_state 用于追踪每个环境是否处于干扰阶段
        self.disturbance_state = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        # remaining_steps 用于追踪干扰的剩余步数
        self.remaining_steps = torch.zeros(self.num_envs, dtype=torch.int, device=self.device)
        ...
        self.disturbance_torque = torch.zeros(self.num_envs, self.num_bodies, 3, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _create_envs(self):
        ...
        if self.cfg.domain_rand.randomize_base_mass:
            self.payload = torch_rand_float(self.cfg.domain_rand.added_mass_range[0], self.cfg.domain_rand.added_mass_range[1], (self.num_envs, 1), device=self.device)
        if self.cfg.domain_rand.randomize_com_displacement:
        ...
        for i in range(self.num_envs):
            ...
            body_props, self.body_mass[i] = self._process_rigid_body_props(body_props, i)
    ...
    def _get_ground_heights(self):
        """ Sample the terrain height of each robot's current point.
            The points are offset by the base's position and rotated by the base's yaw
        ...
        points = self.root_states[..., :2].clone().reshape(self.num_envs, 1, 2)
        points += self.terrain.cfg.border_size
        points = (points / self.terrain.cfg.horizontal_scale).long()
        px = points[:, :, 0].view(-1)
        py = points[:, :, 1].view(-1)
        ...
        return heights.view(self.num_envs) * self.terrain.cfg.vertical_scale
    ...
    def _reward_orientation(self):
        ...
        """
        return torch.exp(-torch.norm(self.base_euler_xyz[:, :2], dim=1) * 5)

    def _reward_base_height(self):
        # Penalize base height away from target
        base_height = self.root_states[:, 2] - self.terrain_h
    ...
    def _reward_energy_cost(self):
        return torch.norm(torch.abs(self.torques) * torch.abs(self.dof_vel), dim=1)
    ...
    def _reward_feet_air_time(self):
        ...
        rew = torch.clamp(rew, max=0.5)
        rew[self.standing_cmd] = 0.0
